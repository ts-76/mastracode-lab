import { loadSettings, MEMORY_GATEWAY_PROVIDER, getAvailableModePacks, getAvailableOmPacks, resolveModelDefaults, resolveOmModel, mastra, releaseThreadLock, acquireThreadLock, getCustomProviderId, toCustomProviderModelId, saveSettings } from './chunk-OXZXGLCJ.js';
...
function discoverDefaultSkillPaths(projectPath) {
  return collectSkillPaths([
    path__default.join(projectPath, ".mastracode", "skills"),
    path__default.join(projectPath, ".claude", "skills"),
    path__default.join(os__default.homedir(), ".mastracode", "skills"),
    path__default.join(os__default.homedir(), ".claude", "skills")
  ]);
}
...
async function createDynamicWorkspace({
  ...
  const allowedPaths = [...resolvedSkillPaths, ...sandboxPaths.map((p) => path__default.resolve(p))];
...
function resolveModel(modelId, options) {
  ...
  if (mgApiKey && isMastraGatewayModel) {
    ...
    return new ModelRouterLanguageModel({ id: `mastra/${normalizedModelId}`, headers }, [
      gateway
    ]);
  }
  ...
  if (isMoonshotModel) {
    ...
    return new ModelRouterLanguageModel({ id: normalizedModelId, headers });
  } else {
    return new ModelRouterLanguageModel({ id: normalizedModelId, headers });
  }
}
...
function createTeamMessageTool(memberId, bus, teamId, emitEvent) {
  return createTool({
    ...
    inputSchema: z$1.object({
      toMemberId: z$1.string().describe('ID of the receiving member, or "broadcast" to send to all'),
      content: z$1.string().describe("Message content")
    }),
...
- You need specialized agents to coordinate via messaging`,
    inputSchema: z$1.object({
      teamId: z$1.enum(teamIds).describe("ID of the team to dispatch"),
      task: z$1.string().describe("The task description. All team members receive the same task.")
    }),
...
var MemberSchema = z$1.object({
  id: z$1.string().describe('Unique member identifier (e.g. "researcher", "implementer")'),
  name: z$1.string().describe("Human-readable display name"),
  instructions: z$1.string().describe("Instructions that guide the member's behavior"),
  defaultModelId: z$1.string().optional().describe(`Override model ID (e.g. "anthropic/claude-sonnet-4-20250514", "openai/gpt-4o"). If omitted, uses the parent agent's current model.`),
  maxSteps: z$1.number().optional().describe("Maximum steps for this member's execution loop")
});
var TeamCreateInputSchema = z$1.object({
  teamName: z$1.string().describe("Name for the new team"),
  description: z$1.string().describe("Brief description of what this team will accomplish"),
  task: z$1.string().describe("The task to dispatch to all team members"),
  members: z$1.array(MemberSchema).min(1).max(8).describe("Team member definitions (1-8 members)"),
  maxConcurrency: z$1.number().optional().describe("Max members running in parallel. Default: all"),
  modelStrategy: z$1.enum(["user_select", "ai_auto", "manual"]).optional().default("manual").describe(
...
async function fetchProvidersFromGateways(gateways = [new ModelsDevGateway({}), new NetlifyGateway(), new MastraGateway()]) {
  const allProviders = {};
  const allModels = {};
...
async function createMastraCode(config) {
  ...
  const mgApiKey = authStorage2.getStoredApiKey(MEMORY_GATEWAY_PROVIDER) ?? process.env["MASTRA_GATEWAY_API_KEY"];
  ...
  const harness = new Harness({
    ...
    modelAuthChecker: (provider) => {
      const gatewayKey = authStorage2.getStoredApiKey(MEMORY_GATEWAY_PROVIDER) ?? process.env["MASTRA_GATEWAY_API_KEY"];
      if (gatewayKey) {
  ...
  return { harness, mcpManager, hookManager, authStorage: authStorage2, resolveModel, storageWarning, extension };
}
