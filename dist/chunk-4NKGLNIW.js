...
var TeamModelPickerComponent = class extends Box {
  searchInput;
  listContainer;
  members;
  allModels;
  ...
  buildMemberList() {
    ...
    for (let i = 0; i < this.members.length; i++) {
      ...
      if (isFocused) {
        ...
        const model = hasSelection ? theme.fg("success", ` \u2192 ${selectedModel}`) : theme.fg("muted", " \u2192 (no model)");
        line = cursor + name + model;
      } else {
        const cursor = "  ";
        const name = hasSelection ? member.name : theme.fg("muted", member.name);
        const model = hasSelection ? theme.fg("dim", ` \u2192 ${selectedModel}`) : theme.fg("muted", " \u2192 (no model)");
        line = cursor + name + model;
      }
  ...
  updateList() {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      const checkmark = isCurrent ? theme.fg("success", " \u2713") : "";
      const noKeyIndicator = !item.hasApiKey ? theme.fg("error", " \u2717") + theme.fg("muted", " (no key)") : "";
      let line;
      if (isSelected) {
        line = theme.fg("accent", "\u2192 " + item.id) + checkmark + noKeyIndicator;
      } else {
...
var TeamActivityComponent = class extends Container {
  ui;
  // Team info
  teamId;
  task;
  ...
  rebuild() {
    this.clear();
    const b = (char) => theme.bold(theme.fg("accent", char));
    const termWidth = getTermWidth();
    const maxLineWidth = termWidth - 6 - BOX_INDENT * 2;
    const teamLabel = theme.bold(theme.fg("toolTitle", "team"));
    const durationStr = this.done ? theme.fg("muted", ` ${formatDuration2(this.durationMs)}`) : "";
    const statusIcon = this.done ? this.anyError() ? theme.fg("error", " \u2717") : theme.fg("success", " \u2713") : theme.fg("muted", " \u22EF");
  ...
  renderOverview(b, _maxLineWidth) {
    ...
    for (const id of this.memberOrder) {
      ...
      if (!m) continue;
      const icon = m.status === "done" ? theme.fg("success", "\u2713") : m.status === "error" ? theme.fg("error", "\u2717") : theme.fg("muted", "\u22EF");
      const modelLabel = m.modelId ? theme.fg("muted", ` (${shortModel(m.modelId)})`) : "";
      const durLabel = m.durationMs != null ? theme.fg("muted", ` ${formatDuration2(m.durationMs)}`) : "";
  ...
  renderMemberDetail(b, maxLineWidth) {
    ...
    if (m.text) {
      ...
      if (!this.expanded && wrappedLines.length > MAX_ACTIVITY_LINES2) {
        this.addChild(new Text(`${b("\u2502")} ${theme.fg("muted", `... ${wrappedLines.length - MAX_ACTIVITY_LINES2} lines above`)}`, BOX_INDENT, 0));
      }
...
var ShellStreamComponent = class extends Container {
  ...
  rebuild() {
    ...
    const durationStr = done ? theme.fg("muted", ` ${formatDuration3(Date.now() - this.startTime)}`) : "";
...
//# sourceMappingURL=chunk-4NKGLNIW.js.map
//# sourceMappingURL=chunk-4NKGLNIW.js.map